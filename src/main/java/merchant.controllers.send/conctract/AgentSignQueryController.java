...
import merchant.beans.contract.AgentSignQueryRequest;
import merchant.beans.contract.AgentSignQueryResponse;
...
public class AgentSignQueryController {
	private Logger logger = LoggerFactory
			.getLogger(AgentSignQueryController.class);
	...
	@Value(value = "${Bridge_TransCode_AgentSignQuery}")
	private String agentSignQuery;
	 ...
	 * @return
	 */
	@RequestMapping(value = "/agentSignQuery.do", method = RequestMethod.POST)
	public String agentSignQuery(Map<String, Object> map, String branchCode,
		...
		// 创建发送请求对象
		AgentSignQueryRequest agentSignQueryRequest = new AgentSignQueryRequest();
		// 以当前时间为时间戳
		String stamp = new SimpleDateFormat("yyyyMMddHHmmssSSS")
				.format(new Date());

		// 发送报文的格式，以json格式传送
		agentSignQueryRequest.setFormat("json");
		// 交易码，固定
		agentSignQueryRequest.getMessage().getHead()
				.setTransCode(agentSignQuery);
		// 上行下送标志，固定
		agentSignQueryRequest.getMessage().getHead().setTransFlag("01");
		// 缴费中心交易序列号
		agentSignQueryRequest.getMessage().getHead()
				.setTransSeqNum("BRIDGE" + stamp + epayCode);
		// 时间戳格式 yyyyMMddHHmmssSSS，
		agentSignQueryRequest.getMessage().getHead().setTimestamp(stamp);

		// 缴费项目配置的主商户在商E付系统的编号
		agentSignQueryRequest.getMessage().getInfo()
				.setAgentSignNo(agentSignNo);
		// 缴费项目编号
		agentSignQueryRequest.getMessage().getInfo().setEpayCode(epayCode);
		// 缴费项目配置的主商户在商E付系统的编号
		agentSignQueryRequest.getMessage().getInfo().setMerchantId(merchantId);
		// 商户交易编号
		agentSignQueryRequest.getMessage().getInfo().setOrderNo(orderNo);

		// 封装完成，将数据转为json格式，然后加密加签名
		String reqJson = JSON.toJSONString(agentSignQueryRequest);
		// 对需要发送的json字符串进行签名
		...
		if (responseStr.startsWith("{\"string\":")) {
			...
			try {
				...
				AgentSignQueryResponse agentSignQueryResponse = JSON
						.parseObject(respJson,
								new TypeReference<AgentSignQueryResponse>() {
								});

				AgentSignQueryResponse.Message.Head head = agentSignQueryResponse
						.getMessage().getHead();
				AgentSignQueryResponse.Message.Info info = agentSignQueryResponse
						.getMessage().getInfo();
				// 返回的消息头信息
				map.put("transCode",
				...
				map.put("agentSignNo", info.getAgentSignNo() == null ? ""
						: info.getAgentSignNo());
				map.put("signDate",
						info.getSignDate() == null ? "" : info.getSignDate());
				map.put("unSignDate",
						info.getUnSignDate() == null ? "" : info
								.getUnSignDate());
				map.put("agentSignStatus",
						info.getAgentSignStatus() == null ? "" : info
								.getAgentSignStatus());
				map.put("certificateNo", info.getCertificateNo() == null ? ""
						: info.getCertificateNo());
				map.put("certificateType",
						info.getCertificateType() == null ? "" : info
								.getCertificateType());
				map.put("last4CardNo ", info.getLast4CardNo() == null ? ""
						: info.getLast4CardNo());
				map.put("cardType",
						info.getCardType() == null ? "" : info.getCardType());
				map.put("merchantId",
				...
				map.put("input1",
						info.getInput1() == null ? "" : info.getInput1());
				map.put("input2",
						info.getInput2() == null ? "" : info.getInput2());
				map.put("input3",
						info.getInput3() == null ? "" : info.getInput3());
				map.put("input4",
						info.getInput4() == null ? "" : info.getInput4());
				map.put("input5",
						info.getInput5() == null ? "" : info.getInput5());
				map.put("invaidDate",
						info.getInvaidDate() == null ? "" : info
								.getInvaidDate());
		...
		return "agentSignQueryResult.jsp";
	}

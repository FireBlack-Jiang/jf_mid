...
public class AgentSignNotifyController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/agentSignNotify.do", method = RequestMethod.POST)
	@ResponseBody
	public void agentSignNotify(String queryRequest,
			HttpServletRequest request, HttpServletResponse httpResponse) {
		String responseJson = null;
		try {
			logger.info("--------进入agentSignNotify----------------------------------");
			// 接收报文
			String requestContent = HttpClientUtils.getRequestBody(request).trim();
			if (logger.isWarnEnabled()) {
				logger.info("-----AgentSignNotifyController------------收到的报文：{}", requestContent);
			}
			String signatureString = requestContent.substring(0,
					requestContent.indexOf("||"));
			logger.info("-----AgentSignNotifyController------------截取报文的signatureString:", signatureString);
			...
			while(m.find()){
				...
				logger.info("-----AgentSignNotifyController------如果有双引号，则截取后的requsetBody:", requsetBody);
			}
			//requsetBody是base64加密后的数据，需解析出来
			String requset = new String(
					com.alibaba.fastjson.util.Base64.decodeFast(requsetBody));
			logger.info("-----AgentSignNotifyController------------解析完成后的requsetBody-------" + requset);
